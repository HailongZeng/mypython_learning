...
一、基本命令
    ...
    quit或exit
    5、查看版本(连接后执行)
    示例：select version();    注意分号不能忽略
    6、显示当前时间
    示例：select now();        注意分号不能忽略
    7、远程连接
...
二、数据库操作
    ...
    示例：create database hailong charset=utf8;
    2、删除数据库
    格式：drop database 数据库名;
    示例：drop database hailong;
    3、切换数据库
    格式：use 数据库名;
    示例：use hailong;
    4、查看当前选择的数据库
...
1、增
    ...
    c、同时插入多条数据：
        ...
        示例：insert into student values(0,'韩梅梅',18,0,'北京',0),
            (0,'Jack',22,1,'海南',0),(0,'Wendy',20,0,'石家庄',0);
2、删
...
3、条件查询
    ...
    b、比较运算符
        ...
        不等于       !=或<>
        需求：查询id值大于2的所有数据
        示例：select * from student where id>2;
    c、逻辑运算符
        and   并且
        or    或者
        not   非
    ...
    f、空判断
        insert into student(name,age) values('trump',70);
        注意：null与''(空字符串)是不同的，null是没有数据，''有数据，只是为空字符串
        判断空：is null
        判断非空：is not null
        需求：查询没有地址的同学
    ...
    g、优先级
        小括号，not,比较运算符，逻辑运算符
        and比or的优先级高，如果同时出现并希望先算or，则需要结合小括号使用
4、聚合
...
5、分组
    ...
    示例：
        ...
        select name,gender,count(*) from student group by name,gender,age;
        select gender,group_concat(name,"-",age) from student group by gender,age;
...
六、关联
    建表语句：
    1、create table class(id int auto_increment primary key, name varchar(20)
       not null, stuNum int not null);
    2、create table students(id int auto_increment primary key, name varchar(20)
       not null, gender bit default 1, classid int not null, foreign key(classid)
       references class(id));
    插入一些数据：
    insert into class values(0,'python01',55),(0,'python02',50),
    (0,'python03',60),(0,'python04',80);
    insert into students values(0,'tom',1,1),(0,'Jack',1,2);

    select * from students;

    关联查询：
    select students.name,class.name from class
    ...
    select students.name,class.name from class
    right join students on class.id=students.classid;
